...
def main(offset=0):
    ...
    for i, stock_basic in enumerate(main_session.query(models.StockBasicPro).all()):
        try:
            ...
            data_frame.loc[i, COL_LIMIT_COUNT_A] = local_limit_count(daily[2:], local_scale=10)
            data_frame.loc[i, COL_LIMIT_COUNT_B] = local_limit_count(daily[2:], local_scale=1)
            # data_frame.loc[i, COL_CONTINUOUS_LIMIT_COUNT] = continuous_limit(daily, local_scale=10)

            data_frame.loc[i, COL_HISTORY_BREAK_INDEX] = daily_break_index(daily[2:], local_scale=30)

            ma_20 = api.daily_close_ma(daily=daily[2:], step=20)
            ...
            if data_frame.loc[i, COL_LIMIT_COUNT_A] == 1 and data_frame.loc[i, COL_LIMIT_COUNT_B] == 1:
                data_frame.loc[i, COL_PASS_1] = True
            else:
                data_frame.loc[i, COL_PASS_1] = False

            if (daily[1].close > daily[1].pre_close) and (daily[1].close < round(daily[1].pre_close * 1.1, 2)) and (daily[1].close > daily[1].open):
                data_frame.loc[i, COL_PASS_2] = True
            else:
                data_frame.loc[i, COL_PASS_2] = False
            ...
            continue
        print('##### analyze_n {i} #####'.format(i=i))

    data_frame = data_frame[
                            (data_frame[COL_PASS_1] == True)
                            & (data_frame[COL_PASS_2] == True)
                            & (data_frame[COL_MA_20_SLOPE] > 0)
                            & (data_frame[COL_HISTORY_BREAK_INDEX] == 0)
                           ]
    ...
    file_name = '{logs_path}/{date}@Analyze_N.csv'.format(date=LAST_MARKET_DATE, logs_path=env.logs_path)
...
def plot_candle_gather(data_frame, last_date, sub, offset):
    ...
    plt.tight_layout()
    plt.savefig('../../buffer/analyze_n/{date}_analyze_n_{sub}.png'.format(date=last_date, sub=sub))
