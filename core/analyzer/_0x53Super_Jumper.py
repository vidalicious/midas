...
COL_JUMP_OFF_CHG = 'COL_JUMP_OFF_CHG'
...
def main(offset=0):
    ...
    for i, stock_basic in enumerate(main_session.query(models.StockBasicPro).all()):
        try:
            ...
            daily_10 = daily[:10]
            daily_closes_10 = [i.close for i in daily_10]
            min_close_10 = min(daily_closes_10)
            min_close_10_index = daily_closes_10.index(min_close_10)

            daily_60 = daily[:60]
            daily_closes_60 = [i.close for i in daily_60]
            max_close_60 = max(daily_closes_60)
            max_close_60_index = daily_closes_60.index(max_close_60)

            if min_close_10_index < max_close_60_index:
                jump_off_chg = (min_close_10 / max_close_60 - 1) * 100
            else:
                jump_off_chg = 0

            data_frame.loc[i, COL_JUMP_OFF_CHG] = jump_off_chg
            ...
            continue
        print('##### super_jumper {i} #####'.format(i=i))

    data_frame = data_frame[
                            (data_frame[COL_JUMP_OFF_CHG] < 0)
                           ]

    data_frame = data_frame.sort_values(by=COL_JUMP_OFF_CHG, ascending=True).reset_index(drop=True)
    data_frame = data_frame.head(100)


    file_name = '{logs_path}/{date}@Super_Jumper.csv'.format(date=LAST_MARKET_DATE, logs_path=env.logs_path)
...
def plot_candle_gather(data_frame, last_date, sub, offset):
    ...
    for i in range(len(data_frame)):
        ...
        misc = {
            ...
            COL_JUMP_OFF_CHG: round(data_frame.loc[i, COL_JUMP_OFF_CHG], 2)
        }
    ...
    plt.tight_layout()
    plt.savefig('../../buffer/super_jumper/{date}_super_jumper_{sub}.png'.format(date=last_date, sub=sub))
...
def plot_candle_month(ax, ts_code, name, last_date, misc):
    ...
    plt.title('{index} {ts_code} {name} circ_mv:{circ_mv}亿 holders:{holders_count} jump_off_chg:{chg}'.format(index=int(misc['index']), ts_code=ts_code, name=name,
                                                                                                                     circ_mv=int(misc[COL_CIRC_MV]), holders_count=int(misc[COL_HOLDERS_COUNT]),
                                                                                                                     chg=misc[COL_JUMP_OFF_CHG]),
