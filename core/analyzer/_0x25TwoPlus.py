...
COL_LIMIT_ACCUMULATION_2 = 'COL_DAILY_AGGRESSIVE_ACCUMULATION_2'
COL_LIMIT_ACCUMULATION_5 = 'COL_DAILY_AGGRESSIVE_ACCUMULATION_5'
...
def main(offset=0):
    ...
    for i, stock_basic in enumerate(main_session.query(models.StockBasicPro).all()):
        try:
            ...
            data_frame.loc[i, COL_LIMIT_ACCUMULATION_2] = round(api.limit_chg_accumulation(daily[:2]), 2)
            data_frame.loc[i, COL_LIMIT_ACCUMULATION_5] = round(api.limit_chg_accumulation(daily[:5]), 2)
    ...
    data_frame = data_frame[
                            (data_frame[COL_LIMIT_ACCUMULATION_2] == data_frame[COL_LIMIT_ACCUMULATION_5])
                           ]
