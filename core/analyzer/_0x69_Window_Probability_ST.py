...
def get_limit_probability(sequence, window_width, local_scale):
...
def main(offset=0):
    ...
    file_name = '{logs_path}/{date}@Window_Probability_ST.csv'.format(date=LAST_MARKET_DATE, logs_path=env.logs_path)
...
def plot_candle_gather(data_frame, last_date, sub, offset):
    ...
    plt.tight_layout()
    plt.savefig('../../buffer/window_probability_st/{date}_window_probability_st_{sub}.png'.format(date=last_date, sub=sub))
