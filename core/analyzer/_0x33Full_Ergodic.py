...
def main(offset=0):
    ...
    for i, stock_basic in enumerate(main_session.query(models.StockBasicPro).all()):
        try:
            ...
            continue
        print('##### full ergodic {i} #####'.format(i=i))
    ...
    file_name = '{logs_path}/{date}@Full_Ergodic.csv'.format(date=LAST_MARKET_DATE, logs_path=env.logs_path)
...
def plot_gether(data_frame, last_date):
    ...
    plt.figure(figsize=(columns * 3, rows * 3 / 2))
    ...
    plt.tight_layout()
    plt.savefig('../../buffer/full_ergodic/{date}_full_ergodic.png'.format(date=last_date))
