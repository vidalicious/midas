...
def main(offset=0):
    ...
    file_name = '{logs_path}/{date}@Minghong_Filter.csv'.format(date=LAST_MARKET_DATE, logs_path=env.logs_path)
...
def plot_candle_gather(data_frame, last_date, sub):
    ...
    plt.tight_layout()
    plt.savefig('../../buffer/minghong_filter/{date}_minghong_filter_{sub}.png'.format(date=last_date, sub=sub))
