...
def main(offset=0):
    ...
    for i, stock_basic in enumerate(main_session.query(models.StockBasicPro).all()):
        try:
            ...
            data_frame.loc[i, COL_LIMIT_COUNT_A] = local_limit_count(daily, local_scale=5)
            data_frame.loc[i, COL_LIMIT_COUNT_B] = local_limit_count(daily, local_scale=1)
            ...
            continue
        print('##### first_limit {i} #####'.format(i=i))
    ...
    data_frame = data_frame.sort_values(by=COL_PRICE, ascending=True).reset_index(drop=True)
    data_frame = data_frame.head(100)


    file_name = '{logs_path}/{date}@First_Limit.csv'.format(date=LAST_MARKET_DATE, logs_path=env.logs_path)
...
def plot_candle_gather(data_frame, last_date, sub, offset):
    ...
    plt.tight_layout()
    plt.savefig('../../buffer/first_limit/{date}_first_limit_{sub}.png'.format(date=last_date, sub=sub))
