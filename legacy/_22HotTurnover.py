...
def main():
    ...
    for key in [COL_AVERAGE_TURNOVER, COL_AVERAGE_P_CHANGE, COL_ACCUMULATE_P_CHANGE, 'circ_mv']:
    ...
    for i, ts_code in enumerate(data_frame.ts_code):
        ...
        try:
            ...
            data_frame.loc[i, COL_AVERAGE_TURNOVER] = api.daily_basic_average_turnover_rate(daily_basic=daily_basic, begin=0, end=end_index)
    ...
    sorted_frame = data_frame.sort_values(by=COL_AVERAGE_TURNOVER, ascending=False)

    file_name = '../logs/{date}@HotTurnover.csv'.format(date=LAST_MARKET_DATE)
    # print(fileName)
