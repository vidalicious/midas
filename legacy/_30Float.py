...
COL_CONTINUOUS_RAISE = 'continuous_raise'
...
def main():
    ...
    for key in [COL_CONTINUOUS_RAISE, COL_AVERAGE_P_CHANGE, COL_ACCUMULATE_P_CHANGE,
                'circ_mv']:
    ...
    for i, ts_code in enumerate(data_frame.ts_code):
        try:
            ...
            continuous_raise = api.daily_continuously_weight_up_count(daily=daily)
            # continuous_up = api.daily_continuously_low_up_count(daily=daily)
            data_frame.loc[i, COL_CONTINUOUS_RAISE] = continuous_raise
            data_frame.loc[i, COL_AVERAGE_P_CHANGE] = api.daily_average_p_change(daily=daily, begin=0, end=1 + continuous_raise)
            data_frame.loc[i, COL_ACCUMULATE_P_CHANGE] = api.daily_accumulate_p_change(daily=daily, begin=0, end=1 + continuous_raise)
    ...
    sorted_frame = data_frame.sort_values(by=COL_CONTINUOUS_RAISE, ascending=True)

    file_name = '../logs/{date}@Float.csv'.format(date=LAST_MARKET_DATE)
    # print(fileName)
