...
COL_WEIGHT_LAST_P_CHANGE = 'WEIGHT_LAST_P_CHANGE'
COL_INFLEXION = 'INFLEXION'
COL_WEEKLY_WEIGHT_LAST_P_CHANGE = 'WEEKLY_WEIGHT_LAST_P_CHANGE'
...
def main():
    ...
    for key in [COL_WEIGHT_CONTINUOUSLY_FALL_P_CHANGE, COL_WEIGHT_LAST_P_CHANGE, COL_INFLEXION,
                COL_WEEKLY_WEIGHT_LAST_P_CHANGE]:
    ...
    for i, ts_code in enumerate(data_frame.ts_code):
        try:
            ...
            data_frame.loc[i, COL_WEIGHT_LAST_P_CHANGE] = api.daily_weight_p_change(daily=daily, begin=0, end=1)
            data_frame.loc[i, COL_INFLEXION] = round(data_frame.loc[i, COL_WEIGHT_LAST_P_CHANGE] - data_frame.loc[i, COL_WEIGHT_CONTINUOUSLY_FALL_P_CHANGE], 2)
            ...
            data_frame.loc[i, COL_WEEKLY_WEIGHT_LAST_P_CHANGE] = api.weekly_weight_p_change(weekly=weekly, begin=0, end=1)
    ...
    data_frame = data_frame[
                           ...
                           & (data_frame[COL_WEIGHT_LAST_P_CHANGE] > 0)
                           & (data_frame[COL_WEEKLY_WEIGHT_LAST_P_CHANGE] > 0)
                           ]

    sorted_frame = data_frame.sort_values(by=COL_WEIGHT_CONTINUOUSLY_FALL_P_CHANGE, ascending=True)
